...
alter table public.articles
  ...
  add column text_tokens text[] not null default '{}',
  add column location_ids text[] not null default '{}',
  add column dedupe_hash text,
  add column source_count integer not null default 1 check (source_count > 0);

update public.articles
set normalized_title = left(trim(regexp_replace(lower(title), '[[:space:][:punct:]]+', ' ', 'g')), 1000),
    normalized_text = left(trim(regexp_replace(lower(description), '[[:space:][:punct:]]+', ' ', 'g')), 5000),
    title_tokens = array(
      select distinct token
      from unnest(regexp_split_to_array(trim(regexp_replace(lower(title), '[[:space:][:punct:]]+', ' ', 'g')), '\s+')) token
      where length(token) >= 3
      order by token
    ),
    text_tokens = array(
      select distinct token
      from unnest(regexp_split_to_array(trim(regexp_replace(lower(description), '[[:space:][:punct:]]+', ' ', 'g')), '\s+')) token
      where length(token) >= 3
      order by token
    ),
...
alter table public.articles
  ...
  add constraint articles_dedupe_hash_format check (dedupe_hash ~ '^[a-f0-9]{64}$');

create index articles_dedupe_window_idx
  on public.articles (category, published_at desc);
create index articles_dedupe_hash_idx
  on public.articles (dedupe_hash);
...
create index article_sources_canonical_url_idx
  on public.article_sources (canonical_url)
  where canonical_url <> '';
create index article_sources_content_hash_idx on public.article_sources (content_hash);
create index article_sources_dedupe_hash_idx on public.article_sources (dedupe_hash);

insert into public.article_sources (
  article_id, source_id, external_id, url, canonical_url, title, description,
  language, content_hash, dedupe_hash, location_ids, published_at,
  first_seen_at, last_seen_at
)
select
  id, source_id, external_id, url, canonical_url, title, description,
  language, content_hash, dedupe_hash, location_ids, published_at,
...
grant select (
  id, article_id, source_id, external_id, url, canonical_url, title,
  description, language, published_at, first_seen_at, last_seen_at
) on table public.article_sources to anon, authenticated;
grant select, insert, update, delete on table public.article_sources to service_role;
grant select, insert, update on table public.articles to service_role;

create policy "Public can read sources of published articles"
  on public.article_sources for select to anon, authenticated
  using (
    exists (
      select 1
      from public.articles article
      where article.id = article_sources.article_id
        and article.is_published = true
    )
  );

create or replace function private.token_jaccard(left_tokens text[], right_tokens text[])
returns numeric
language sql
immutable
parallel safe
...
as $function$
  with left_set as (
    select distinct token from unnest(coalesce(left_tokens, '{}'::text[])) token
  ), right_set as (
    select distinct token from unnest(coalesce(right_tokens, '{}'::text[])) token
  ), intersection_size as (
    select count(*)::numeric as value
    from left_set
    join right_set using (token)
  ), union_size as (
    select count(*)::numeric as value
    from (select token from left_set union select token from right_set) combined
  )
  select case when union_size.value = 0 then 0
    else intersection_size.value / union_size.value end
  from intersection_size, union_size;
$function$;

revoke all on function private.token_jaccard(text[], text[]) from public, anon, authenticated;
grant execute on function private.token_jaccard(text[], text[]) to service_role;
...
begin
  ...
  if v_article_id is not null then
    ...
    set external_id = p_external_id,
        ...
        last_seen_at = now()
    where article_id = v_article_id and source_id = p_source_id;
