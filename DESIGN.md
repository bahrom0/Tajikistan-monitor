...
- Demo data: `src/data/news-demo.ts` is frontend-only and marks every item `demo: true`; it covers critical, high, medium, low, official, media, recommended, regional and city cases. `NewsPage` owns the presentation contract so the future adapter can replace the fixture without changing the visual component tree.
...
- Images: `public/news-demo-khujand.jpg` is the local authorized demo fallback; remote demo URLs are allowed to fail into that stable-size fallback. Images retain `object-fit: cover`, descriptive alt text and a fixed aspect ratio to avoid layout shift.
